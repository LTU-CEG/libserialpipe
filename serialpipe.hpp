    ...
    while (!shutdown)
    {
      ...
      try
      {
        if (_string_data)
        {
          ...
          {
            _rx_buffer.insert(_rx_buffer.begin(), data.begin(), data.end());
...
public:
  /**
   * @brief   Constructor for the bridge. Setups the serial
   ...
   */
  bridge(std::string port, unsigned int baudrate,
      ...
      : _string_data(string_data), _string_termination(string_termination)
  {
    ...
    serial::Timeout timeout(inter_byte_timeout, timeout_ms, 0, timeout_ms, 0);

    /* Set the input port to "" in order to not automatically open. */
    _serial = std::unique_ptr< serial::Serial >{
        new serial::Serial("", baudrate, timeout)};
  ...
  ~bridge()
  {
    ...
    _tx_dowork.notify_all();
    _serial_wait.notify_all();
    _cb_dowork.notify_all();
